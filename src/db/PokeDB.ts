...
const syncData = async () => {
  try {
    ...
    for (const p of data.pokemonmon) {
    ...
    emit(3, 4, 'Locations');
    for (const l of data.locationsations) {
      void lStore.put({
        ...DEFAULT_LOCATION,
        ...l,
        parent: l.parent, // stay undefined if omitted
      });
    }
