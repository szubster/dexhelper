    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-3 relative z-10">
          {(() => {
            const staticEnc =
              staticEncounters[pokemonId]?.[
                gameVersion as keyof (typeof staticEncounters)[number]
              ];
            const versionEnc = encounters.filter((e: LocationAreaEncounter) =>
              e.version_details.some(
                (v: VersionEncounterDetail) => v.version.name === gameVersion,
              ),
            );

            if (
              (staticEnc && staticEnc.length > 0) ||
              versionEnc.length > 0 ||
              evoReq
            ) {
              return (
                <>
                  ...
                  {versionEnc.map((e: LocationAreaEncounter, i: number) => {
                    const versionDetail = e.version_details.find(
                      (v: VersionEncounterDetail) =>
                        v.version.name === gameVersion,
                    );
                    return (
                      <div
                        key={i}
