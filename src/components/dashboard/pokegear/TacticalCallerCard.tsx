...
export function TacticalCallerCard({ contact, type, details, probability }: TacticalCallerCardProps) {
  ...
  return (
    <div
      className={`tactical-panel group relative flex flex-col gap-2 border-2 bg-black/60 p-3 text-xs ${themeColors.border}`}
    >
      ...
      <div
        className={`tactical-panel absolute top-0 right-0 border-t-0 border-r-0 border-b-2 border-l-2 px-2 py-1 text-[9px] ${themeColors.probBg} ${themeColors.probBorder} ${themeColors.probText}`}
      >
        PROB: {probability}%
      </div>
      ...
      <div className="flex w-full flex-col gap-2 pt-2">
        ...
        {(type === 'SWARM' || type === 'ITEM') && (
          <div className="z-10 mt-1 flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <span
                className={`tactical-panel tactical-text px-1.5 py-0.5 text-[9px] ${themeColors.badgeBg} ${themeColors.badgeText}`}
              >
                [ {type} ]
              </span>
