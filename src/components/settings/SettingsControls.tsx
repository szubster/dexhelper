  ...
  return (
    <div className="space-y-4">
      <SettingsRow
        ...
        label="Version"
      >
        <SegmentedControl className="grid grid-cols-3 gap-2" ariaLabel="Game Version">
          {versions.map((v) => (
            <SegmentedControlButton
              key={v.id}
              onClick={() => setManualVersion(v.id === 'unknown' ? null : v.id)}
              isActive={effectiveVersion === v.id || (v.id === 'unknown' && effectiveVersion === 'unknown')}
              activeClassName="border border-dashed border-blue-500 bg-blue-500/20 text-blue-400 shadow-[0_0_10px_rgba(59,130,246,0.3)] focus-visible:ring-blue-500"
              inactiveClassName="border border-dashed border-zinc-800 bg-zinc-950 text-zinc-500 hover:border-zinc-600 hover:bg-zinc-900 hover:text-zinc-300 focus-visible:ring-blue-500"
              className="px-3"
            >
              {v.label}
            </SegmentedControlButton>
      ...
      <SettingsRow
        ...
        label="Living Dex"
      >
        <SegmentedControl ariaLabel="Living Dex Mode">
          <SegmentedControlButton
            onClick={() => setIsLivingDex(false)}
            isActive={!isLivingDex}
            activeClassName="bg-zinc-800 text-white"
          >
            [ STANDARD ]
          </SegmentedControlButton>
          <SegmentedControlButton onClick={() => setIsLivingDex(true)} isActive={isLivingDex}>
            [ LIVING DEX ]
          </SegmentedControlButton>
        </SegmentedControl>
      </SettingsRow>
      ...
      <SettingsRow
        ...
        label="Ball Style"
      >
        <SegmentedControl className="grid grid-cols-3 gap-2" ariaLabel="Ball Style">
          {filteredPokeballs.map((pb) => (
            <SegmentedControlButton
              key={pb.value}
              onClick={() => setGlobalPokeball(pb.value)}
              isActive={globalPokeball === pb.value}
              activeClassName="border border-dashed border-amber-500 bg-amber-500/20 text-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.3)] focus-visible:ring-amber-500"
              inactiveClassName="border border-dashed border-zinc-800 bg-zinc-950 text-zinc-500 hover:border-zinc-600 hover:bg-zinc-900 hover:text-zinc-300 focus-visible:ring-amber-500"
