...
export function useFileSyncController() {
  ...
  const processFile = useCallback(
    async (file: File) => {
      try {
        ...
        console.error('System: parse live save failed');
        setStatus('error');
  ...
  useEffect(() => {
    async function restoreHandle() {
      try {
        ...
        console.error('System: restore handle failed');
      }
    }
    void restoreHandle();
  }, [processFile]);
  ...
  const resumeSync = useCallback(async () => {
    try {
      ...
      console.error('System: resume sync failed');
      setStatus('error');
    }
  }, [processFile]);
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      try {
        ...
        console.error('System: polling failed');
