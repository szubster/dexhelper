...
describe('generateSuggestions', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(evoGen, 'generateEvolutionSuggestions').mockImplementation(() => {});
  });

  it('handles null saveData or apiData', () => {
    const result1 = generateSuggestions(null, false, undefined, mockApiData, mockStrategy);
    expect(result1.suggestions).toEqual([]);

    const result2 = generateSuggestions(mockSaveData, false, undefined, null, mockStrategy);
    expect(result2.suggestions).toEqual([]);
  });

  it('generates missing ids and skips mewtwo when hof count is 0 in gen 1', () => {
    mockSaveData.hallOfFameCount = 0;
    mockSaveData.owned = new Set(Array.from({ length: 149 }, (_, i) => i + 1));

    const result = generateSuggestions(mockSaveData, false, undefined, mockApiData, mockStrategy);
  ...
  it('caps queryTargets to 100', () => {
    mockSaveData.owned = new Set();

    generateSuggestions(mockSaveData, false, undefined, mockApiData, mockStrategy);
  ...
  it('filters by missing tools as a post process', () => {
  ...
  it('identifies myOtIds based on trainerName', () => {
  ...
  it('groups instances by species for subgenerators', () => {
    mockSaveData.partyDetails = [
      { speciesId: 12, otName: 'ASH' },
