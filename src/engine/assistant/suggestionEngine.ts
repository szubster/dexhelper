 ...
 *    `myOtIds`, and `instancesBySpecies`. This prevents O(N) array scans during the intensive generation loops.
 * 2. **Dependency Injection**: Specific generation mechanics are handled by the injected `AssistantStrategy`
 *    and generator functions, keeping this main loop clean and free of heavy `if (gen === X)` branching.
