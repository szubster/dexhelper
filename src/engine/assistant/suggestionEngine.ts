 ...
 */
export async function fetchAssistantApiData(
  saveData: SaveData,
  queryTargets: number[],
) {
  let localSlug = "";
  ...
  const missingPromises = queryTargets.map(async (pid: number) => {
    try {
      ...
      if (ancestors.length > 0) {
        ...
        await Promise.all(
          ancestors.map(async (aid) => {
            const aEncs = await pokeapi.resource(
  ...
  const partyPromises = (saveData.party || []).map(async (pid: number) => {
    try {
      const species = await pokeapi.resource(
...
export function generateSuggestions(
  ...
  if (apiData.localEncounters) {
    ...
    for (const encounter of apiData.localEncounters) {
      ...
      if (isFiniteEncounter && myOtIds.has(pid)) {
        rejected.push({
          pokemonId: pid,
          reason:
      ...
      if (saveData.eventFlags && gift?.eventFlag) {
        if (
          (saveData.eventFlags[Math.floor(gift.eventFlag / 8)]! &
            (1 << (gift.eventFlag % 8))) !==
          0
        ) {
          rejected.push({
      ...
      if (versionDetail && missingIds.includes(pid)) {
        localPids.push(pid);
        localEncounterInfo[pid] = versionDetail.encounter_details.map(
          (ed: any) => ({
            chance: ed.chance,
  ...
  if (apiData.missingEncounters) {
    const locationMap: Record<
      string,
      { name: string; distance: number; pids: Set<number>; slug: string }
    > = {};
    let unobtainableCount = 0;

    for (const pid of queryTargets) {
      const isFiniteEncounter = !!STATIC_GIFT_DATA[pid];
      if (isFiniteEncounter && myOtIds.has(pid)) continue;
    ...
    locations.slice(0, 4).forEach((loc) => {
      ...
      pids.forEach((pid) => {
        ...
        const areaEncounter = pidEncounters.find(
          (enc: any) =>
        ...
        if (areaEncounter) {
          const versionDetail = areaEncounter.version_details.find(
            (vd: any) => vd.version.name === displayVersion,
          );
          if (versionDetail) {
            locEncounterInfo[pid] = versionDetail.encounter_details.map(
              (ed: any) => ({
                chance: ed.chance,
                method: ed.method.name,
                minLevel: ed.min_level,
