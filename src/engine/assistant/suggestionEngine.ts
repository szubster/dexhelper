...
export function generateSuggestions(
  ...
  const myOtIds = new Set(allInstances.filter(p => p.otName === saveData.trainerName).map((p: PokemonInstance) => p.speciesId));
  ...
  for (const trade of STATIC_NPC_TRADE_DATA) {
    ...
    const isClaimed = allInstances.some((p: PokemonInstance) =>
