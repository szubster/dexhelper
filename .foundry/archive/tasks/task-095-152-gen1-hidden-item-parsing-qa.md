...
- [x] Verify that `hiddenItemFlags?: Uint8Array` has been added to the `SaveData` interface in `src/engine/saveParser/parsers/common.ts`.
- [x] Verify that the `parseGen1` function correctly calculates the offset and extracts the hidden item flags.
- [x] Verify that the implementation correctly uses the `offsetShift` when calculating the hidden item flags offset to ensure Pokemon Yellow compatibility.
