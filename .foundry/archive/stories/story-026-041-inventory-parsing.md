---
id: story-026-041-inventory-parsing
type: STORY
title: 'Story 1: Detailed Inventory Parsing'
status: "COMPLETED"
owner_persona: tech_lead
created_at: '2026-05-06'
updated_at: "2026-05-05"
depends_on: []
jules_session_id: null
pr_number: null
parent: .foundry/epics/epic-015-026-save-parser-expansion.md
tags:
  - gen2
  - save-parser
  - inventory
research_references: []
rejection_count: 0
rejection_reason: ''
notes: ''
---

# Story 1: Detailed Inventory Parsing

## Context
The Gen 2 Save Parser Expansion Epic requires detailed inventory parsing to provide accurate data for the engine.

## Objectives
- Implement the missing data extraction layer for detailed inventory parsing.

## Acceptance Criteria
- [x] Able to extract Key Items.
- [x] Able to extract Special Rods.
- [x] Able to extract TM/HMs (Headbutt, Rock Smash).
- [x] Able to extract Apricorns.
- [x] Able to extract Evolution Items.

### Child Tasks
- `.foundry/archive/tasks/task-041-066-implement-inventory-parsing.md`
- `.foundry/archive/tasks/task-041-067-qa-inventory-parsing.md`
