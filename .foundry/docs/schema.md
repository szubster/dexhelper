...
confidence_score: null  # Optional. Integer (0-100) allowing agents to self-report their confidence levels.
...
| `confidence_score` | `integer` | optional | Integer (0-100) allowing agents to self-report their confidence levels. |
...
7. **VERIFYING and COMPLETED nodes are read-only for implementing personas.** Once a PR is merged, the node transitions to `VERIFYING`. Once verified, it becomes `COMPLETED` and must not be edited. The TPM archives it.
