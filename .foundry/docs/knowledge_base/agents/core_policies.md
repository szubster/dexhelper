...
- **Mandate Decomposition, Granularity, and Late Binding**: To ensure predictable execution and maximum pipeline throughput, all generative personas (PM, Epic Planner, Story Owner, Tech Lead) MUST actively decompose broad concepts into multiple, smaller, highly-focused downstream nodes rather than single monolithic nodes or 1-to-1 mappings.
  ...
  - **Map Dependencies**: If one generated task depends on the implementation details of another, you MUST explicitly set the `depends_on` field of the dependent task to point to the prerequisite task to prevent DAG deadlocks.
  - **Define Clear Contracts**: Generated tasks should serve as a clear contract for the implementer. Include necessary context, constraints, and acceptance criteria.
