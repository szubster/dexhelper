...
- **Mandate Decomposition, Granularity, and Late Binding**: To ensure predictable execution and maximum pipeline throughput, all generative personas (PM, Epic Planner, Story Owner, Tech Lead) MUST actively decompose broad concepts into multiple, smaller, highly-focused downstream nodes rather than single monolithic nodes or 1-to-1 mappings.
  ...
  - **WIP Integration & Draft Architectures**: Generative personas (Epic Planner, Story Owner, Tech Lead) MUST actively verify the status of architectural blueprints and code dependencies. Do NOT mandate the integration of Draft ADRs (`status: DRAFT`) or WIP/Draft PRs into production pipelines. Draft ADRs are explicitly exploratory and subject to breaking changes. Generative nodes must either wait for Draft ADRs to reach `APPROVED`/`IMPLEMENTED` state, or explicitly note that the implementation is experimental and should be isolated from stable features.
...
- **Playwright Navigation Paths**: Playwright tests and code examples must use relative paths (e.g., `./dashboard`) for navigation rather than absolute paths (e.g., `/dashboard`) to prevent test failures caused by Vite base URL configurations.
