...
- [x] Implement logic in `foundry-orchestrator.ts` and `foundry-engine.yml` so every agent automatically retrieves and includes all research references from its parent chain.