...
## Requirements
- Verify that manual dispatch of a scheduled workflow creates an issue with the correct labels (`jules`), title pattern, and body payload containing the compiled prompt.
- Verify that the issue creation correctly triggers Jules via the native issue listening functionality.
- Verify that when the Jules PR merges, the issue is automatically closed natively.
