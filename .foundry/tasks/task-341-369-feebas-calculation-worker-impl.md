...
depends_on: []
jules_session_id: '14916519939592874837'
pr_number: null
parent: null
tags:
  - gen3
...
## Acceptance Criteria
- [ ] Create a new Web Worker script (e.g. `src/engine/saveParser/parsers/feebas.worker.ts`) that imports and calls `calculateFeebasTiles(seed)`.
...
- [ ] Ensure that all guidelines from Section 13 ("Save File Parsing & Extraction Guidelines") in `.foundry/docs/schema.md` are strictly adhered to (no magic numbers, proper relative offsets, explicit bitwise mapping).
- [ ] Ensure `src/engine/saveParser/parsers/gen3.test.ts` is updated and all tests continue to pass.
