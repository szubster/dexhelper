...
parent: ./stories/story-009-composite-deprecation
tags: []
rejection_count: 0
notes: ""
---
...
## Context
As defined in Story 009, we need to explicitly deprecate the concept of "composite nodes" in `./docs/schema.md`. Since the term doesn't currently exist, we must add an explicit rule forbidding it to prevent confusion. Composite nodes are an anti-pattern because they lead to DAG deadlocks in our orchestrated workflow.

## Requirements
1. Update `./docs/schema.md` to explicitly flag "Composite Nodes" as an anti-pattern.
...
## Acceptance Criteria
- [x] "Composite node" terminology is explicitly flagged as an anti-pattern in `./docs/schema.md`.
