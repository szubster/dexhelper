...
depends_on: []
jules_session_id: '7949996834448508241'
pr_number: null
...
## Acceptance Criteria
- [ ] Add logic in `.github/scripts/foundry-orchestrator.ts` to check if a dependency node has `status === 'DRAFT' || status === 'WIP'`.
- [ ] Emit a warning and set `hasUnresolvableDeps = true` (and `shouldSuspend = true` / `blocked = true`) if the condition is met.
