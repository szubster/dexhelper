...
- The script must read the YAML frontmatter of `**/*.md` files and validate that any repo-relative paths specified in the `depends_on` and `parent` fields exist on the filesystem.
...
## Context
This task addresses the requirements outlined in `./stories/story-006-015-implement-link-checker.md`. Ensure that the implementation respects the project's strict TS and linting rules if writing a Node.js script.
