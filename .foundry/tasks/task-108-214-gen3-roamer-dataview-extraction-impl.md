---
id: task-108-214-gen3-roamer-dataview-extraction-impl
type: TASK
...
## Objective
Implement the logic to extract the Gen 3 Roamer data structure (20 bytes) from a save file using `DataView`, and parse out its IVs, HP, and Level, explicit testing boundary states for bitwise operations per ADR 026.
...
**Bitwise State Extraction Constraint:** Per ADR 026, parsing IVs requires explicit bitwise masking and shifting. You must also include comprehensive boundary testing (all zeros, max values, and edge boundary conditions).
...
- [ ] Parse the 32-bit IV field into 6 distinct integer stats (HP, Atk, Def, Spe, SpA, SpD) using explicit bitwise shifts and masks.
- [ ] Parse the Level and Current HP fields correctly.
- [ ] Include unit tests explicitly validating data extraction against boundary conditions for bitwise extraction (e.g., all 0 IVs, all 31 IVs) per ADR 026.
