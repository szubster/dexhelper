...
## Objective
Implement a core backend algorithm (`src/engine/breeding/pair_algorithm.ts` or similar) that takes a user's Pokémon storage as input, cross-references Gen 2 Egg Groups, Genders, and Shiny Carrier flags, and outputs a list of valid breeding pairs prioritized by Shiny Carrier status. This implementation must correctly handle the Gen 2 DV overlap breeding constraints.

## Technical Contract
- Create a function (e.g., `calculateBreedingPairs(pokemonList: Pokemon[])`) that returns an array of `BreedingPair` objects.
- A valid pair in Gen 2 must share at least one Egg Group and have opposite genders.
- **CRITICAL CONSTRAINT**: The algorithm must correctly enforce the Gen 2 rule that prevents two Pokémon from breeding if they are considered "related" due to overlapping DVs.
