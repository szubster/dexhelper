...
depends_on: []
jules_session_id: '12604466888960027377'
pr_number: null
...
## Objectives
- Identify the correct Game Boy Advance / Gen 3 memory offsets or save block structure where berry patch data is stored.
- Determine the structure of a single berry patch entry (e.g., how map ID, berry ID, growth stage, time planted, and last watered time are laid out in bytes).
- Investigate how this data aligns with real-time clock (RTC) mechanics.
- Produce a clear mapping that the `coder` persona can use to implement the `DataView` parsing logic.
