...
## 2026-05-19: Handling Permanent Node Failures
When a leaf task (e.g., `task-062-100-gen3-locations-script-impl`) fails permanently by reaching its `Max Rejection Count` (3), a new set of implementation and QA tasks must be created to resolve it. Furthermore, it's necessary to spawn a `RESEARCH` node to investigate the root cause of the failure before attempting implementation again. In this case, `task-062-101-gen3-locations-script-qa` was set to `CANCELLED` and `research-062-122-gen3-locations-script-failure`, `task-062-123-gen3-locations-script-impl`, and `task-062-124-gen3-locations-script-qa` were spawned, correctly establishing dependencies to enforce sequential execution.
