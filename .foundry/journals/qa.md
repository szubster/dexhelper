...
## Section Verification for Gen 3 Save Blocks (2026-06-12)
**Lesson**: When validating save parsing tasks for Gen 3, QA must strictly verify that the documented Section ID (e.g., Section 2 vs Section 3-4) correctly aligns with the specific block offset. Mislabeling the Section can lead to catastrophic parsing errors if the offset is interpreted relative to the wrong block boundary. A recent task incorrectly mapped the Mirage Island offset to Section 3-4 instead of Section 2.
