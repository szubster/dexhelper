...
- **2026-06-11**: If `pnpm test` fails due to Playwright browsers being missing (e.g. `browserType.launch: Executable doesn't exist`), run `pnpm exec playwright install chromium` before running tests. Similarly, if `lefthook` fails during installation in pre-commit setup, resetting `git config --unset-all --global core.hooksPath` unblocks the install process.
