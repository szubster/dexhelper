...
## Missing Memory Offsets or Constraints (Late Binding Pattern)
When specific constraints, memory offsets, or bitfield formulas are missing from the task definition or knowledge base, DO NOT hallucinate fallback limits or formulas. Implementing generic bounds or guessing exact parsing logic violates the core groundedness rule and risks save corruption.

**Action:** Following the Late Binding pattern, spawn a `RESEARCH` node to investigate and document the missing values. Append this node to the implementation task's `depends_on` array as an unchecked dependency. Then, permanently fail the implementation task (setting `status: FAILED` with a clear `rejection_reason` in the YAML frontmatter) to suspend it in the Orchestrator until the research is complete.
