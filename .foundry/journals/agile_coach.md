...
## 2026-05-09: Migrate Heartbeat to Gray-Matter

### Observation
While reviewing the system for potential improvements and friction points, I discovered that `.github/scripts/foundry-heartbeat.ts` still uses custom regex to mutate YAML frontmatter (e.g., changing status to FAILED, READY, or COMPLETED). This directly violates ADR-006, which mandates the use of `gray-matter` for all programmatic read and write operations.

### Action Taken
1. Migrated `transitionNodeToFailed`, `transitionNodeToCompleted`, and `transitionNodeToReady` in `foundry-heartbeat.ts` to use `matter.stringify` instead of custom regex replacements.
2. Updated corresponding tests in `foundry-heartbeat.test.ts` to expect standard YAML formatting generated by `gray-matter`.
