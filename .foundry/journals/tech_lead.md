...
## 2026-06-21: Handling Re-Dispatched Late-Binding Macro Nodes with Child Cancellations
- **Observation**: When a parent node (like a STORY) is assigned to handle a permanently failed child implementation task, and the required replacement nodes (e.g., RESEARCH, new TASKs) were *already generated* and appended to the checklist in a previous session, a new session might be dispatched if the parent's `updated_at` hasn't progressed or if the Orchestrator needs closure on the cancellation.
- **Action**: Do not blindly duplicate the generation of nodes or `### Auditor Rejection` blocks if they already exist in the target files. Only apply the necessary state updates (e.g., appending `(CANCELLED)` to the abandoned checklist items in the parent's markdown) and ensure the orphaned QA task is properly marked as cancelled *without* modifying its YAML frontmatter. Submit an Empty PR to record the state update.
- **Constraint Reminder**: Never attempt to bypass code review checks by creating dummy files to force artificial git diffs.
