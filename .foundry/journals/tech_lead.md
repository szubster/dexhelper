...
- **Incident**: The implementation task `task-105-197-gen3-roamer-parser-impl` failed permanently because extracting the Gen 3 roamer's map group and map number from the `.sav` file is mathematically impossible (as documented in `adr-108-027-gen3-roamer-location-impossible`), but the technical blueprint still demanded it.
- **Action**: Spawned a new `RESEARCH` node (`research-105-211-gen3-roamer-parser-failure.md`) to explicitly confirm this limitation for this specific parser workflow. Created replacement implementation and QA tasks (`task-105-214` and `task-105-215`) that omit the map extraction requirements. Appended these new nodes to the parent story and added an `### Auditor Rejection` cancellation note to the orphaned QA task (`task-105-198-gen3-roamer-parser-qa.md`) without modifying its YAML frontmatter.
- **Lesson**: Always verify data availability through existing research or ADRs before creating technical blueprints that mandate the extraction of specific data points.
