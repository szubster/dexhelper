...
## 2026-06-24: Cancelled Child Node Parent Update Rule
- **Incident**: The tech lead was checking off a permanently cancelled child node from the parent's markdown, but memory needed to be enforced.
- **Lesson**: When a child node is permanently cancelled, its corresponding checkbox in the parent node's markdown body MUST be checked off (`- [x]`) to allow parent completion. New replacement nodes must be appended as unchecked tasks (`- [ ]`).
