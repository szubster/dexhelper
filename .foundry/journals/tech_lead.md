...
### 2026-06-22 - Handling Permanent Child Task Failures
When a child node reaches its Max Rejection Count and permanently fails, the correct procedure is to spawn a `RESEARCH` node to investigate the root cause, create replacement nodes that explicitly depend on the new `RESEARCH` node, and append the new nodes as unchecked tasks to the parent's markdown body. Do not update the YAML frontmatter of orphaned pending QA task nodes; instead, mark them as CANCELLED via an `### Auditor Rejection` block in their Markdown body linking to the new replacement tasks.
