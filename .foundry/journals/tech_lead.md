...
## Handling Permanent Failures and Orphaned QA Tasks
*   **Incident:** Implementation tasks occasionally fail permanently (Max rejection count reached) due to incorrect specifications or missing architectural constraints.
*   **Action:** When creating a replacement retry task, spawn a `RESEARCH` node (if required to document the missing constraints) and dynamically link the new implementation task to it. Append the newly generated child nodes to the parent story's markdown body WITHOUT modifying its YAML frontmatter. Ensure the orphaned QA task from the previous run is correctly cancelled.
