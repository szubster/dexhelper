...
## 2026-06-14: Handling Implicit Data and Impossible Tasks (Gen 3 Berries)
- **Observation**: A coder task (`task-095-157`) permanently failed because its acceptance criteria required extracting "Time Planted" and "Last Watered Time" for Gen 3 Berries. These values are not explicitly stored in the save data, making the task impossible.
- **Action**: When addressing permanent failures caused by implicit or missing data requirements, spawn a `RESEARCH` node to clarify what data is actually available. Then, create replacement Coder and QA tasks that explicitly reference the research node and strictly instruct the exclusion of the impossible fields to prevent another failure loop. Ensure the orphaned QA task is explicitly marked CANCELLED in its markdown body, and parent stories are appended with the new task references.
