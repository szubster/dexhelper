...
## 2026-06-21: Handling Permanent Failure of Dynamic Item List Parsing
- **Incident**: The implementation task `task-128-181-implement-item-list-parsing` failed permanently (Max rejection count reached) because the coder failed to update the vite configuration (`vite-plugins/pokedata-plugin.ts`) to include the newly generated `items.jsonl` payload into the client bundle, which was an architectural requirement dictated by ADR-049-025.
- **Action**: Set the status of the target task to `CANCELLED` and recorded its failure. Spawned a new `RESEARCH` node (`research-128-210-item-list-parsing-failure.md`) to explicitly document this missing integration constraint. Created replacement implementation and QA tasks (`task-128-212` and `task-128-213`) focusing on the missed vite plugin updates. Appended these new tasks to the parent story and added a cancellation note to the orphaned QA task (`task-128-182-qa-item-list-parsing.md`) without touching its YAML frontmatter.
- **Lesson**: When blueprints require artifacts to be made available to the client, the tech lead must explicitly detail the necessary scaffolding, integrations, or build-time plugin configuration required to bridge the gap. Relying purely on the data generation step is insufficient if the client cannot ingest it.
