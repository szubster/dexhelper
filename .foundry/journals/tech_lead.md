...
## 2026-06-21: Handling Permanent Failure of Dynamic Item List Generation
- **Incident:** The implementation task `task-128-181-implement-item-list-parsing` failed permanently (Max rejection count reached) because the coder failed to update `vite-plugins/pokedata-plugin.ts` to include the new `items.jsonl` data file into the final `pokedata.msgpack` payload, despite it being an explicit requirement in `ADR-049-025`.
- **Action:** Spawned a `RESEARCH` node (`research-128-212-missing-vite-plugin-integration.md`) to explicitly document the required Vite plugin modifications. Created replacement implementation and QA tasks (`task-128-213` and `task-128-214`) that depend on the research node. Appended the newly generated child nodes to the parent story markdown without modifying its YAML frontmatter, and marked the failed task nodes as CANCELLED. Appended a cancellation note to the orphaned QA task (`task-128-182-qa-item-list-parsing.md`) without touching its frontmatter.
