...
## Orchestrator vs UI Synchronization
Orchestrator state machine changes (like introducing implicit dependencies) must always be paired with corresponding DAG UI/Dashboard updates. Otherwise, we introduce silent state changes that confuse users relying on the visual tools to understand task flow.
