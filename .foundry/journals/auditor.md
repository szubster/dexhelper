...
## 2026-06-01: Clarification on IDEA Node Verification
While macro nodes like Epics and Stories require strict hierarchical completion enforcement (as noted on 2026-05-24), high-level planning nodes like IDEAs are an exception. For IDEA nodes, verification must only confirm that the appropriate downstream artifacts (e.g., PRDs) were successfully generated and linked. We should not fail IDEA nodes simply because the final end-to-end code implementation does not yet exist. This ensures planning nodes can be completed and archived appropriately once their specific responsibilities are fulfilled.
