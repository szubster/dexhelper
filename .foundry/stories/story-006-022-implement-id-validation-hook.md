...
parent: ./epics/epic-006-id-pre-commit-hooks
---

# Implement ID Validation Pre-commit Hook

## Objective
Implement a pre-commit hook that parses all `**/*.md` files (excluding journals and docs) to validate the integrity of their ID fields as defined in the master schema.
...
- [x] The hook must verify that every `id` field is unique globally within the `` directory.
...
### Child Nodes
- `./tasks/task-022-039-implement-id-validation-hook.md`
- `./tasks/task-022-040-qa-id-validation-hook.md`
