...
## Problem
Currently, visual differences in components (especially complex interactive interfaces like the DAG canvas, node visualizers, and dashboard layouts) are difficult to track, review, and assert locally. The project team previously integrated Argos CI, but rapidly exhausted the free-tier limits, and there is no budget or desire to pay for commercial SaaS visual regression services. Developers need a free, self-hosted, deterministic way to verify visual rendering locally and within CI without depending on third-party SaaS platforms.
...
2. Add specific, automated screenshot tests focusing on critical user interfaces, especially the interactive DAG visualization Canvas (`@xyflow/react`) and the core application dashboards.
...
## Acceptance Criteria
- [ ] Product Manager: Draft a PRD defining the local visual regression setup, screenshot strategies for the canvas, and artifact upload workflows for CI.
