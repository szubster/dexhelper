...
## 2026-05-18 - Gen 2 Gift/Trade Suggestions Isolation\n**Learning:** When generating suggestions, we were using a shallow merge of gen 1 and gen 2 gift data `STATIC_GIFT_DATA`, meaning the same object would have different contexts. E.g. spearow might be a static gift in gen 2, but a normal catch in gen 1, resulting in cross generation leaks. If a Gen 1 player catches a Spearow but needs another for a Living Dex, the assistant will incorrectly suppress the wild encounter suggestion because Spearow exists in the Gen 2 gift dataset. \n**Action:** Replaced shallow merge of the static gift and trade data constants with dynamic selection using the parsed `saveData.generation` to ensure context appropriate recommendations.
