...
## 2026-05-12 - Assistant Roaming Legendary Catch Suggestion
**Learning:** The suggestion engine was ignoring roaming legendaries (e.g., Raikou, Entei, Suicune) extracted from Gen 2 save data. The player could be on the exact same map as a roaming beast without receiving a recommendation to catch it.
**Action:** Implemented a check against `saveData.roamingLegendaries` to generate a critical priority (130) catch suggestion when the player's `currentMapId` matches the roamer's `mapId`. This provides immediate, actionable feedback to hunt down the elusive legendary.
