...
## 2024-05-20 - Assistant Field Move Check
**Learning:** In Generation 2 mechanics, Headbutt (TM02) and Rock Smash (TM08) are single-use TMs, not HMs. They do not require gym badges to be used in the field. Furthermore, because they disappear from the inventory when taught to a Pokémon, the suggestion engine must check if any Pokémon in the party or PC knows the move (Headbutt ID: 29, Rock Smash ID: 249) rather than solely relying on TM inventory checks to prevent hiding valid field encounter suggestions.
**Action:** Always check `partyDetails` and `pcDetails` for move IDs 29 (Headbutt) and 249 (Rock Smash) in addition to inventory checks when evaluating field move encounters.
