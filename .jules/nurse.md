...
## $(date +%Y-%m-%d) - Nurse: Removed unsafe `as keyof typeof` casts for dictionary key access
**Learning:** When checking if a dynamic string key exists in a strictly typed imported JSON dictionary (like `mapLocations.json`), using `key in dict ? dict[key as keyof typeof dict] : fallback` is an unsafe cast that fights the TypeScript compiler.
**Action:** Replaced inline `as keyof typeof` casts with a generic custom type guard `export function isValidKey<T extends object>(key: PropertyKey, obj: T): key is keyof T { return key in obj; }` which leverages TypeScript's native narrowing safely without needing explicit overrides.
