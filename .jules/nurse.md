...
- When using generic React components like `TacticalSegmentedControl`, explicitly pass generic type arguments (e.g., `<TacticalSegmentedControl<Type>>`) to leverage TypeScript inference and eliminate the need for manual `as` casts in event handlers.
