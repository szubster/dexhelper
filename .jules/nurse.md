## 2026-05-08 - Typed `CATEGORY_STYLES` with `SuggestionCategory`
...
## 2026-05-08 - Nurse: Replaced inline string unions and array casting with `as const` derived array and type
...
## 2026-05-08 - Nurse: Tighten React Props to remove type casting
**Learning:** By keeping React component prop types as strict as their underlying models (e.g. `GameVersion` instead of `string`), we can avoid using unsafe casts like `as keyof ...` when indexing into static data objects.
**Action:** Changed `gameVersion: string` to `gameVersion: GameVersion` in `PokemonDetailsProps` and `PokemonLocationsProps` to eliminate the cast.
