...
Learned that when working on React components, vitest browser tests require assertions like 'await expect.element(page.getByText(...)).toBeVisible();' inside each 'it' block to satisfy the oxlint 'vitest(expect-expect)' rule.
