...
\n## 2026-06-27 - Type-Safety: `Object.values` casts\n**Learning:** When retrieving the values of a typed `const` object map, `Object.values(MAP) as readonly Type[]` is an unsafe cast that circumvents the type checker and can become inaccurate if the underlying object shape shifts. \n**Action:** Use a type-inferring helper function like `objectValues` (e.g. `export function objectValues<K extends string, V>(obj: Partial<Record<K, V>>): V[]`) that wraps `Object.values` to preserve the value types without an explicit `as` cast.
## $(date +%Y-%m-%d) - Type-Safety: `as` casts in `Object.keys` mapping
**Learning:** Using `(Object.keys(MAP) as Array<keyof Type>).map((key) => { ... })` and then casting the resulting properties (e.g., `variant={item.variant as 'amber' | 'white' | 'default'}`) bypasses TypeScript's safety mechanisms. The compiler cannot guarantee that the mapped properties match the explicit literal union type, leading to unsafe runtime assumptions.
**Action:** Replace `Object.keys` with the `objectKeys` utility (`src/utils/object.ts`) which properly infers the keys. Explicitly type the derived variables inside the `.map` callback (e.g., `const variant: 'amber' | 'white' | 'default' = ...`) before passing them down as props. This removes the need for down-casting `as` entirely and ensures the compiler enforces the literal union type constraint.
- ReactFlow components often rely on `any` types for node `data` props. When removing them, define strict interfaces extending `Record<string, unknown>` to ensure compatibility with generic node handlers while maintaining strict typing for expected properties.
## $(date +%Y-%m-%d) - Type-Safety: `as GameVersion` in Gen3 Parser
**Learning:** In `src/engine/saveParser/parsers/gen3.ts`, `gameVersion` was typed as `string` forcing downstream casts like `gameVersion as GameVersion` when passed to `parseSecretBaseRecord`.
**Action:** Changed the type of `gameVersion` parameter from `string` to `GameVersion` in `parseGen3SecretBases` and `parseGen3Roamer`. This removes the need for `as GameVersion` casts inside the function and pushes type safety up to the caller where `_forcedVersion` or inferred values exist.
