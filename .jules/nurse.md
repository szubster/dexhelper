...
## $(date +%Y-%m-%d) - Type-Safety: `as` casts in component callbacks
**Learning:** React generic UI components (like `TacticalMultiSelectControl` and `TacticalSegmentedControl`) are sometimes written without the generic argument populated at the call site, which forces downstream callbacks like `onValueChange` and `onValueToggle` to fallback to wider types or unions like `string | number`. Developers then hack around this by wrapping the setter in an inline arrow function that forcibly `as` casts the argument.
**Action:** When using generic React components, explicitly pass generic type arguments (e.g., `<TacticalMultiSelectControl<(typeof FILTER_TYPES)[number]>>`) to leverage TypeScript inference. This automatically types the internal callback value, allowing you to completely eliminate the inline cast and simplify the callback to just `onValueToggle={toggleFilter}`.
