...
- When making performance optimizations involving React hooks like `useMemo`, ensure the rules of hooks are followed by not calling hooks conditionally. Specifically, ensure any `if` conditions that return early are placed *after* all hook calls.
