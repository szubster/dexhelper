...
- Empty PR Empty Diff Context: If a scribe task requires documenting specific code modules, and those modules are already thoroughly and comprehensively documented with JSDoc, architecture notes, and module-level summaries, do not invent redundant or meaningless documentation gaps. State that no actionable work is found and submit an empty PR.
