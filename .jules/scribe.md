...
## 2026-06-25 - Gen 3 Memory Architecture, Berry Patch, and Roamer Constraints

- **A/B Bank Flash Memory:** Gen 3 uses an alternating 56KB block system. Future parsers *must* explicitly scan both `0x0000` and `0xE000` for the `0x08012025` signature and compare `saveIndex` values rather than assuming a linear file structure, or they risk rolling back the user's data to an older save state.
- **Densely Packed Bitwise Structures (Berry Patches):** Many Gen 3 structures, such as Berry Patches, aggressively pack state into individual bits (e.g., a single byte tracking both the `0x7F` growth stage and a `0x80` boolean stop flag). Modifying or reading these requires strict bitmasking rather than direct byte assignment.
- **Dynamic Roamer Block Offsets:** The memory location for roaming Pokémon data in SaveBlock1 is *not* static across Gen 3. It shifts based on the length of preceding data arrays. Parsers must dynamically adjust the offset (+0x3144 for R/S, +0x31DC for Emerald, +0x30D0 for FR/LG) before attempting to unpack the 32-bit IV structure, otherwise it will result in corrupted stat readings.
## 2026-06-25 - Documentation Verification Complete

**What:** Performed a comprehensive review of the documentation within the focus areas (`engine/saveParser`, `engine/assistant/suggestionEngine.ts`, `engine/mapGraph`, `store.ts`, `scripts/`). No actionable documentation gaps were identified.
**Why:** The prompt requested identifying the least-documented high-complexity module and adding JSDoc or architectural documentation. However, an exhaustive inspection revealed that all critical paths, including the `A/B Bank flash memory`, `Berry Patch` bitwise packing, Gen 1 `detectVersionAndOffsets` logic, `parseGen3Roamer` version shifts, and `Floyd-Warshall` map graph lookups, are already fully and accurately documented with inline JSDocs, parameter definitions, and detailed architecture notes. Proceeding with an Empty PR to prevent the session from being flagged as abandoned, adhering to the Empty PR policy.
