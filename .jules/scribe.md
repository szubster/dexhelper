...
## 2024-05-20 - Gen 1/Gen 2 Save Parser Memory Mapping

**What:** Added JSDoc for `parseGen1` and `parseGen2` in `src/engine/saveParser/parsers`.
**Why:** Gen 1 saves lack explicit version bytes, requiring heuristic detection. Yellow version shifts many memory offsets by +1 byte, requiring dynamic probing to determine the correct alignment before extracting data via `detectVersionAndOffsets`. Gen 2 memory offsets differ significantly between Gold/Silver and Crystal. The parser dynamically uses an `isCrystal` flag to selectively parse extra caught data (time, level, location) that is exclusive to Pokemon Crystal.
