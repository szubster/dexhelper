...
## 2026-06-05 - ETL Pipeline Documentation

**What:** Added module-level JSDoc and inline comments explaining the *why* behind logic in `scripts/generate-pokedata.ts`.
**Why:** The data pipeline script is responsible for transforming and compacting raw PokeAPI JSON into highly optimized, flattened structures for IndexedDB. It lacked documentation on *why* certain non-obvious steps existed:
- **Compaction:** Explicitly documented why removing default values (like `tr: 1` or empty arrays) was critical to preventing IndexedDB bloat.
- **Floyd-Warshall Graph Generation:** Explained that precomputing the All-Pairs Shortest Path distance matrix at build time prevents locking the main UI thread during runtime graph traversals.
- **Indoor Location Reconciliation:** Documented why recursively identifying the "parent" outdoor map of an indoor map (like a house) is necessary for distance lookups.
Documenting these architecture decisions directly in the ETL script ensures future developers understand the boundaries between offline preprocessing and the synchronous recommendation engine.
