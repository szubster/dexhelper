...
Instead of a direct implementation due to the complexity and risk of regression across the DAG dashboard features, I created an `IDEA` node (`idea-418-replace-xyflow-with-custom-dag`) proposing a custom, lightweight SVG/DOM rendering implementation to replace `@xyflow/react` while retaining `dagre` for layout coordinates. This follows a similar successful optimization previously applied to `BattleFrontierDashboard`.