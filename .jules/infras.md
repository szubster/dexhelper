...
## 2026-06-03 - Replaced hardcoded node-version with .nvmrc in GitHub Actions
**Learning:** Evaluated the `.github/workflows/` directory and found that all workflow files (`ci.yml`, `deploy.yml`, `foundry-engine.yml`, `playwright.yml`, `sync-pokedata.yml`) had hardcoded `node-version: 24` or `node-version: '24'`. This violates best practices because upgrading Node.js would require modifying many files, increasing the risk of missing one and causing build breaks. Replaced the hardcoded versions with `node-version-file: ".nvmrc"` so that the project maintains a single source of truth for the Node.js version, making future upgrades atomic and foolproof.
