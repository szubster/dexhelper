...
## 2026-07-08 - React Compiler Boundary
**Learning:** Evaluated adding `babel-plugin-react-compiler` to Vite configuration to automatically handle memoization. The change was rejected because it violates the strict boundary of "focus on the ecosystem, not the app itself." The React compiler modifies application runtime logic, which is outside the scope of the developer tooling and build pipeline improvements expected from the Infras persona. Additionally, any new tooling must explicitly integrate with GitHub (Actions, PR checks, etc.), which a Vite configuration change alone does not fulfill.
