...
## 2026-07-08 - React Compiler Boundary Fix
**Learning:** Initially evaluated adding `babel-plugin-react-compiler` to Vite configuration but faced CI lockfile duplication issues and boundary violations. After user clarification, the React compiler is considered infra and within scope since it changes how the app is built without altering product behavior. The plugin was correctly integrated by adding it to `devDependencies` and configuring `@vitejs/plugin-react` with the `babel` option, properly bypassing TypeScript errors with `@ts-expect-error` where necessary.
