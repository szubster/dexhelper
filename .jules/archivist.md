...
## 2026-06-16 - Archivist Run Learnings
**Learning:** Transient node state transition updates ("is now VERIFYING", "Resurrection Loop triggered") generated by automated orchestration loops continually leak into `.foundry/journals/tpm.md`. This spam violates the core policy against logging transient status updates in journals, leading to severe context bloat over hundreds of lines.
**Action:** Cleared out over 500 lines of transient orchestrator logs from `tpm.md` to conserve context and adhere to the core policy.
