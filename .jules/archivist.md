...
**Learning:** Case sensitivity issues (e.g., `.Jules` vs `.jules`) can cause duplication of agent private_memories, making it harder to track critical learnings.
**Action:** Ensure that all tools and scripts consistently use lowercase for `.jules` directories.

**Learning:** Agent private_memories tend to duplicate learning for recurring accessible patterns over time (e.g., `aria-label` missing on icon buttons, `role="switch"` missing on custom toggles, etc.).
**Action:** Periodically review and consolidate private_memory entries into a single comprehensive entry per topic to avoid repetition.
