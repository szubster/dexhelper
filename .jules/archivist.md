...
## Stale Memory Removal Learnings
- **Scoping Memory Cleansups**: When searching for stale memories, explicitly target files detailing past infrastructure state (e.g. V1 to V2 transitions, completed offline modernization, V1 order-and-architect plans) that have already been fully superseded. Merged PRs and resolved migrations should be aggressively pruned to keep the agent context window lean. Do not overwrite existing journal entries when logging new learnings, as this deletes critical long-term memory.
