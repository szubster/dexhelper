...
- In `vitest-browser-react` browser tests (Vitest 4), the `vitest(expect-expect)` lint rule enforced by `oxlint` requires tests to contain at least one explicit assertion using the browser API, such as `await expect.element(page.getByText(...)).toBeVisible();`. Rendering alone is insufficient and causes CI lint failures.
