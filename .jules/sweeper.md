...
## 2026-06-25 - Knip Dead Code Analysis and Verification
**Learning:** When using `knip --production` to find dead code, files like `src/utils/dag/readFoundryFiles.ts` may be legitimately unused if they were created for earlier backend-centric DAG parsing solutions that were later superseded by Vite plugins (e.g. `vite-plugins/foundry-plugin.ts`). However, always verify that no open or pending `.foundry/` tasks rely on them.
**Action:** When finding unused utility files, search the `.foundry/` directory to ensure they aren't part of an ongoing implementation before deleting them. Delete their associated `.test.ts` files as well to maintain a clean test suite.
