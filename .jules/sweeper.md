...
>> When using `knip` to find unused files/exports, double-check if they are used by external scripts (like `.github/workflows` or unindexed `package.json` scripts) before deleting them. `knip` is currently configured to only look at specific files and may not have visibility into github workflow scripts. For example, `.github/scripts/verify-dag-refs.ts` was flagged as unused by knip, but it was actively used by `.github/workflows/foundry-engine.yml` and `verify:refs` script in `.github/scripts/package.json`. In such cases, add the file to the `ignore` array in `knip.json` instead of deleting it.
