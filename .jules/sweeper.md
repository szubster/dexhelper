...
## 2026-06-03 - Unlinked Components and Dead Code
**Learning:** Components created for features (like the Run Dashboard UI) that are never integrated into the application hierarchy become unlinked dead code. Even if they pass QA individually or unit tests, if they are not imported by any reachable part of the application, they should be cleaned up.
**Action:** When finding unlinked components, verify their status (e.g. via Foundry tasks) to see if their implementation was abandoned or failed integration. If they are permanently unlinked, remove them to simplify the codebase. Always verify with global search (`grep`) to ensure no implicit usage exists before deletion.
