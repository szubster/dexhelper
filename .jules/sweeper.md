...
\n## 2026-05-28 - Removing unused code needs extra verification\n**Learning:** When trying to remove dead code identified by static analysis tools (like knip), you must use global search (grep) to ensure they are not dynamically referenced. Specifically, do not delete unused files if they are referenced in dynamic strings like `import('./gen' + generation + '/assistantData')` or are data exports used across multiple modules.
