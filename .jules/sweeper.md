...
## 2026-06-25 - Identifying Unused Production Exports with Knip

**Learning:** By default, `pnpm knip` evaluates the entire repository, meaning that if an export is only used in a test file (e.g., `AliveTeamView.test.tsx`), Knip will consider it "used." To accurately identify dead code or abandoned utility functions that are not used in the main application logic, run `pnpm knip --production`. Before removing flagged code, search the `.foundry/tasks/` directory for its context to confirm it isn't part of an ongoing or recently closed task.
