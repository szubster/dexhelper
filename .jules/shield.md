...
## Sanitize Error Logging (CWE-209)\n**Pattern:** When resolving CWE-209 by replacing `console.error(err)` with static strings, fully omit the caught error variable (e.g., `catch {`) rather than prefixing it with an underscore (e.g., `catch (_err)`) to avoid unused variable lint warnings enforced by Biome/oxlint.
