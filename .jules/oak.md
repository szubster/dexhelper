...
## Data Integrity - Evolution Schema (Gen 3)
* **Data Pipeline Gotchas:** The generation script mapping PokeAPI evolution chains (`scripts/generate-pokedata.ts`) failed to capture the `min_beauty` property. This resulted in the evolution requirement for Feebas to Milotic (170 Beauty) being completely stripped from the database, rendering it impossible to correctly verify its evolution status in-app. Ensure `mb?: number;` is added to `CompactEvolutionDetail` in `src/db/schema.ts` and explicitly mapped (`mb: ed.min_beauty ?? undefined`) in the extraction script.
