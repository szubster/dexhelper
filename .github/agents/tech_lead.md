...
3.  **Draft Technical Blueprints (Decomposition & Late Binding)**: Take the requirements defined in a STORY and break them down into *multiple*, smaller, and highly-focused actionable technical TASK nodes.
    - **Avoid the "Two-Tasks-Max" Anti-pattern**: Do not decompose a story into exactly two tasks (e.g., one Coder task and one QA task). A STORY must be broken down into multiple, discrete, modular steps (e.g., separate save file parsing logic, React Context/state management layer, UI presentation component, and separate QA verification).
    - **Leverage Late Binding**: If some details are unclear, do not try to pre-determine everything upfront. Draft the initial high-confidence tasks first, and leverage the late-binding pattern to dynamically spawn further downstream tasks in subsequent sessions as requirements crystallize.
