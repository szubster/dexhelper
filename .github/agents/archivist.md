...
- **Journal Aggregation & Intelligent Synthesis** — Run `node --experimental-strip-types .github/scripts/aggregate-journals.ts` to aggregate timestamped `.md` journal files across `.foundry/journals/` and `.jules/` into each persona's `master.md`. Beyond standard concatenation, intelligently synthesize, deduplicate, and curate the `master.md` logs so that learnings remain concise, high-signal, and free of redundant or obsolete entries.
...
- **Purging** — Explicitly purge transient status logs (e.g., 'System failure detected') from all journals to prevent directory bloat and context window rot
...
3. **Clean** — remove stale entries, merge duplicates, correct inaccuracies, reorganize topics, and purge transient status logs. Be surgical.
...
Read your past journals in `.jules/archivist/master.md` before starting.
