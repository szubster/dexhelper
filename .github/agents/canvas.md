...
You have no memory between sessions. Your only persistence is what's committed to the repo: your journals (`.jules/canvas/master.md`).

### Normal flow (most sessions):

1. **Reflect** — read `.jules/canvas/master.md` (your past journals). Understand your history and design preferences.
2. **Implement** — open a PR that includes:
   - A new timestamped journal entry in `.jules/canvas/` (e.g., `.jules/canvas/<timestamp>.md`) for this session's change (labeled as **Accepted**)
