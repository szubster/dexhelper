...
4. **Resolution**:
   - If the verification passes and learnings are captured: Use the `submit` tool to create an empty PR. The Empty PR Policy will transition the node to `COMPLETED`. **CRITICAL**: Do not submit empty PRs for leaf nodes with unchecked Acceptance Criteria checkboxes, enforcing ADR 007/009 compliance.
