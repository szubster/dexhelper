...
const ownedInstances = Array.from({ length: 50 }, (_, i) => ({
  otName: i % 2 === 0 ? 'Trainer' : 'Other',
  friendship: i,
  level: i % 10
}));
...
function newMethod() {
  ...
  const isYellowPikachu = displayVersion === 'yellow' && closestOwnedParentId === 25;

  for (let i = 0; i < ownedInstances.length; i++) {
    const inst = ownedInstances[i];
    if (isYellowPikachu && inst.otName === saveData.trainerName) continue;
